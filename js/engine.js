...
var Engine = (function(global) {
		...
		if (play === true) {
			...
			for (row = 0; row < numRows; row++) {
				for (col = 0; col < numCols; col++) {
					 ...
					 */
					ctx.drawImage(Resources.get(rowImages[row]), col * 101, row * 83, 101, 120);
				}
			}
			renderEntities();
		}
	...
	function menu() {
		...
		for (var i = 0; i < chars.length; i++) {
			ctx.drawImage(Resources.get(chars[i]), i * 101 + 101, 158, 101, 150);
		}

		ctx.fillStyle = 'white';
		ctx.textAlign = 'center';
		ctx.font = 'bold 20pt Times New Roman';
		ctx.fillText('Choose a character, then press enter to begin', ctx.canvas.width / 2, 325);
